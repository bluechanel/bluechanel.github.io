---
title: Airflow使用
pageId: db974ca7-8565-4823-b420-12b1b204e0a9
...
[https://airflow.apache.org/docs/apache-airflow/2.8.0/docker-compose.yaml](https://airflow.apache.org/docs/apache-airflow/2.8.0/docker-compose.yaml)
...
{% mark color="red" %}airflow的默认时区是UTC{% /mark %}，这也是[官方](https://airflow.apache.org/docs/apache-airflow/stable/timezone.html)推荐的，其schedule也是以UTC时区存储和使用，将时区的转换交给了DAG编写者控制
...
我们在大陆地区，设置DAG的运行时间为每天早晨8点({% mark color="red" %}UTC+8，上海时区{% /mark %})，如下图所示，任务实际运行时间为{% mark color="red" %}2023-01-09 08:00:00(UTC+8 时区){% /mark %}， 逻辑时间是前一天{% mark color="red" %}2023-01-08 08:00:00(UTC+8 时区){% /mark %}， 运行日志文件名为{% mark color="red" %}xx_2023-01-08 00:00:00(UTC 时区){% /mark %}， 这个时候代码中使用的airflow变量`{{ ds }}`的值为 {% mark color="red" %}2023-01-08(UTC){% /mark %}
...
如果我们设置DAG运行时间为每天凌晨0点({% mark color="red" %}UTC+8，上海时区{% /mark %})，那么我们在19号早上查看记录，任务实际运行时间为{% mark color="red" %}2022-09-19 00:00:00(UTC+8 时区){% /mark %} ，逻辑时间是前一天 {% mark color="red" %}2022-09-18 00:00:00(UTC+8 时区)， {% /mark %}运行日志文件名为{% mark color="red" %}xx_2022-09-17 16:00:00(UTC 时区){% /mark %}，这个时候代码中使用的airflow变量`{{ ds }}`的值为 {% mark color="red" %}2022-09-17(UTC){% /mark %}
